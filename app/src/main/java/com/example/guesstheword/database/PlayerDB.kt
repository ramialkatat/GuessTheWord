...
abstract class PlayerDB:RoomDatabase() //This class is abstract because Room creates the implementation for it and not us.An abstract class cannot be instantiated. Means, we cannot create object of abstract class.
{
    ...
    companion object{ //allows clients to access the methods for creating or getting the DB without instantiating the class. Since the only purpose of this class is to provide us with a database, there's no reason to instantiate it
        ...
        private var INSTANCE: PlayerDB? = null //Instance will keep a reference for the DB once we have one. This will help us avoid repeatedly opening connections to the database, which is expensive.
        fun getInstance(context: Context):PlayerDB //this method returns a reference to the Player Database. We're going to use a builder which requires a context, so we pass that context in and we want to return a PlayerDB
        {
            synchronized(this){//multiple threads can potentially ask for a DB instance at the same time in more complex apps. Wrapping our code into synchronized means that only one thread of execution at a time enters this block of code, to ensure that the DB is initialized only once
               var instance=INSTANCE //copying the current value of Instance to a local variable, and using smart casting
               if(instance==null)
               {
                   instance= Room.databaseBuilder( //creating the database by first invoking the databaseBuilder
                       context.applicationContext, //supply the contexts that we passed in
                       PlayerDB::class.java, //telling it which DB to build
                       "player_data_database" //giving the DB a name
                   )
